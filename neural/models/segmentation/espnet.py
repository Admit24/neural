...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.level1 = ConvBNReLU(in_channels[0], out_channels, 1)
        self.level2 = ConvBNReLU(in_channels[1], out_channels, 1)
        self.level3 = ConvBNReLU(in_channels[2], out_channels, 1)

        self.up1 = nn.ConvTranspose2d(out_channels, out_channels, 2, stride=2)
        self.up2 = DeConvBNReLU(out_channels, out_channels)
        self.up3 = DeConvBNReLU(out_channels, out_channels)

        self.conv1 = ConvBNReLU(out_channels * 2, out_channels, 1)
...
def DeConvBNReLU(in_channels, out_channels):
    return nn.Sequential(
