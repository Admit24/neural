...
class WDSR(nn.Module):
    def __init__(self, in_channels, out_channels,
                 scale_factor, num_channels=32, num_blocks=8, resblock=None):

        super().__init__()

        self.head = weight_norm(nn.Conv2d(
            in_channels, num_channels, 3, padding=1))
        ...
        self.upscaling = nn.Sequential(
            nn.Conv2d(num_channels, out_channels * scale_factor ** 2, 3, padding=1),
            nn.PixelShuffle(scale_factor),
        )

        self.skip = nn.Sequential(
            weight_norm(nn.Conv2d(in_channels, out_channels * scale_factor ** 2, 5, padding=2)),
            nn.PixelShuffle(scale_factor),
        )
...
class ResidualBlockA(nn.Module):
    def __init__(self, in_channels, out_channels, expansion_ratio, scaling=None):
        super().__init__()

        self.scaling = scaling
        expansion_channels = in_channels * expansion_ratio

        self.conv1 = weight_norm(nn.Conv2d(in_channels, expansion_channels, 3, padding=1))
        self.conv2 = weight_norm(nn.Conv2d(expansion_channels, out_channels, 3, padding=1))
